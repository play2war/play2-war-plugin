...
@RunWith(classOf[JUnitRunner])
class Tomcat7029Tests extends AbstractTomcat7x with Java6 {
...
class Jetty9xTests extends AbstractPlay2WarTests with Servlet30Container with Java7 {
  ...
  // override def containerUrl = "http://repo1.maven.org/maven2/org/eclipse/jetty/jetty-distribution/9.0.0.M1/jetty-distribution-9.0.0.M1.tar.gz"
  override def containerFileNameInCloudbeesCache = Option("jetty-distribution-9.0.0.M0.tar.gz")
  // override def containerFileNameInCloudbeesCache = Option("jetty-distribution-9.0.0.M1.tar.gz")
